...
public class StrengthPowerup : MonoBehaviour {
	...
	public CircleCollider2D SpawnAreaC;
	public bool Spawned;
	...
	void Start () {
		...
		SpawnArea = GameObject.Find ("PowerupSpawnArea");
		SpawnAreaC = SpawnArea.GetComponent<CircleCollider2D> ();
	...
	void OnTriggerEnter2D( Collider2D collider2D ) {
		if (collider2D.gameObject.Equals (Player2)) {
			...
			StrPwr.transform.position = new Vector3 (1000, 1000);
			Spawned = false;
		} else if (collider2D.gameObject.Equals (SpawnAreaC)) {
			Spawned = true;
		}
	}
}
